...
void create_dns_query(unsigned char *host, int query_type, unsigned char *buf, size_t *question_len ,int *query_len)
{
	...
	*query_len = sizeof(struct dns_header) + (*question_len + 1) + sizeof(struct question);
}

void parse_dns(struct event *ev)
{
	struct resolver_result* result = (struct resolver_result *)ev->ev_arg;
	...
	int i  = 0;
	if (ev->timeout) {
		result->handler((void *)result);	
	}

	nread = read(ev->ev_fd, buf, 65536);
	...
	result->value = (char **)calloc(dns.ancount, sizeof(char *));	
	for (i = 0, tmp = dns.answers; tmp; tmp =tmp->next) {
		if (tmp->type != 1) {
			continue;
		}	
		result->value[i] = strdup(tmp->data);
		i++;
	}
